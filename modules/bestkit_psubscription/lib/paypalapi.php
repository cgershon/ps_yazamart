...
class paypal_recurring
{
			...
			if ($addBillingFreq) {
				...
				//	$request .= '&TOTALBILLINGCYCLES=' . urlencode($period->billing_cycles);     ADD YGPC 9/04/2016 
        		 	 if( (int)$_SESSION['billing_cycles']  > 0 )
        		  		{     
	        		 	 	$request .= '&TOTALBILLINGCYCLES=' . urlencode( $_SESSION['billing_cycles']  - 1 );
						 // Important fixing the amount of each cycle        	
							$request .= '&AMT=' . urlencode( $this->paymentAmount) ; //  also added by YGPC not included in 
	          			//	$request .='&INITAMT='. urlencode( $this->paymentAmount +  $_SESSION['decimale_part_excl'] ) ;  
	          												// ADD decimale part down rounded to the first payment  
						}
					
			//	var_dump(  "&INITAMT=", urlencode( $this->paymentAmount +  $_SESSION['decimale_part_excl'] ) );exit; 	
	...
	{	//echo 'setExpressCheckout';
		//var_dump( 'setExpressCheckout'  );exit; 	
		$nvpStr = 
		"&PAYMENTREQUEST_0_AMT=".$this->paymentAmount
	//	.'&INITAMT='. urlencode( $this->paymentAmount +  $_SESSION['decimale_part_excl'] )	
		...
		//	var_dump( '	$nvpStr  ',	$nvpStr ,'     ORDER: ',$this->generatePaymentRequest($this->order));exit; 	
	...
	{	//echo 'fn_createRecurringPaymentsProfile';
	...
	{	//echo 'fn_getExpressCheckout';
		//var_dump( '	$nvpStr  ',	$nvpStr_  );exit; 	
		$version = urlencode('104.0');
